...
**Status: PENDING ENGINEER** — spec delivered 2026-04-20 by ux-designer at `docs/tui-ux-specs/fb-102-activity-unavailable-recovery-affordance.md`. **Option D selected** — compact parenthetical inline in S3 body, transient-only: `"activity unavailable (press [r])"` where `[r]` renders accent+bold, parenthetical in muted. CRD-absent keeps clean `"activity unavailable"` (distinguished by absence of parenthetical). Implementation — 3 sites: (1) `resourcetable.go` adds `activityCRDAbsent bool` field + `SetActivityCRDAbsent(bool)` setter + clear in `SetActivityRows()` (auto-reset on row arrival, no extra wire-up) + gate at `renderActivitySection()` line ~304; (2) `model.go:471` adds `m.table.SetActivityCRDAbsent(isCRDAbsent)` after `SetActivityFetchFailed(true)` (uses already-computed `isCRDAbsent` local at line 464). Coordinates with FB-100: no bundling — FB-100 handles populated-rows case (stale rows retained), FB-102 handles empty-rows case (inline copy); complementary surfaces, no overlap. 9 ACs covering Observable (transient vs CRD-absent), Input-changed, Anti-behavior (normal state; rows-arrival clears), Anti-regression (FB-082 + FB-076), Integration. Originally filed 2026-04-20 by product-experience from FB-082 user-persona P3-2.
...
**Status: PENDING ENGINEER 2026-04-20** — ux-designer spec delivered 2026-04-20 at `docs/tui-ux-specs/fb-116-orientation-hint-false-quickjump.md`. Decision: Option A — `"→  select a resource type from the sidebar to get started"` (single substring changed at `resourcetable.go:644`). Rationale over B: Option B's post-registration phase provides zero marginal information — when `len(m.registrations) > 0`, `renderQuickJumpSection()` already renders quick-jump keys visibly below the hint; two-phase copy would restate what the operator already sees. Option B also requires expanding gate condition with internal branching, adding complexity for no net signal gain. Hint is most valuable as calm single directive at maximum-disorientation (zero registrations); once registrations arrive interface communicates the rest. Spec includes explicit §4 calling out FB-105 test-anchor update as intentional (same pattern as FB-115 → FB-079). ACs cover Observable ×2 (false clause absent, correct directive present), Input-changed, Anti-regression ×3 (FB-105 anchor, forceDashboard suppression, org-scope suppression), Integration. **Queued behind FB-114 + FB-115 feat/console commits.** **Prior Status: PENDING UX-DESIGNER 2026-04-20** — designer-call required on copy fix (Option A minimal strip vs Option B stateful two-phase hint). Filed 2026-04-20 by product-experience from FB-105 user-persona P2 finding. Persona interpretation verified in code.
