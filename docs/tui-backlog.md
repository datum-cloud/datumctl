...
**Status: ACCEPTED + PERSONA-EVAL-COMPLETE (2026-04-20)** — implementation in `model.go:1265-1271` cancels the queued open: clears `pendingQuotaOpen`, resets the strip cancel label, clears the hint, and zeroes `quotaOriginPane` (FB-095 invariant). Hint copy aligns with FB-115's `"…press [3] to cancel"` (which superseded FB-079's `"when ready"`). 7 tests in `model_test.go` cover all 6 ACs across Repeat-press / Observable / Input-changed / Anti-behavior / Anti-regression / Integration axes; `go install ./...` clean; `go test ./internal/tui/...` green. **Persona eval 2026-04-20:** 0 P1/P2/P3 findings. 6 positive findings confirm: cancel gesture is natural muscle-memory (same key starts and cancels the queued action); `"Quota dashboard loading… press [3] to cancel"` is self-teaching on first encounter; `"Quota dashboard cancelled"` is proportionate and matches the unified cancel-acknowledgement vocabulary used across nav-cancel, esc-cancel, and second-press cancel; cancel-then-restart flow leaves no residue (origin zeroed, strip clear, post-load no-op hint); nav-cancel / esc-cancel / second-press cancel share the same string — operator learns the pattern once. Spec delivered 2026-04-19 by ux-designer. Designer chose Option C: second `3` press while pending cancels the queue. Filed 2026-04-19 by product-experience from FB-047 user-persona P3-4.
...
**Status: PERSONA-EVAL-COMPLETE 2026-04-20** — filed 2026-04-19 by product-experience from FB-042 user-persona P3-4. Persona eval clean; one P3 copy-polish finding folded into FB-139.
...
**Status: ACCEPTED + PERSONA-EVAL-COMPLETE (2026-04-20)** — implementation at `model.go:2094-2106` extends `placeholderActionRow` signature with `contentW int`; below the `contentW < 40` threshold the retry copy falls back to `" retry"`, otherwise `" retry describe"`. Call site at `model.go:2129` passes `m.detail.Width()`. Threshold value matches FB-140's welcome sub-line narrow-suppression cutoff — one consistent narrow-width boundary across the codebase. 3 FB-106 tests cover AC1/AC2 Observable (wide + narrow) and AC3 Input-changed (wide→narrow→wide reversibility) using `stripANSIModel(m.buildDetailContent())` View-level assertions. Anti-regression: `TestFB084_AC1_NonRetryable_RKeyAbsentFromView` + `TestFB084_AC2_Retryable_RKeyAndRetryDescribeInView` green. `go install ./...` clean; full TUI suite `-count=1` green. **Persona eval 2026-04-20:** 0 P1/P2/P3 findings. 5 positive findings confirm: short form `[r] retry` is unambiguous in narrow-pane context (viewport itself provides the disambiguation the qualifier supplied in text at wider widths); `[E] events  [r] retry  [Esc] back` ordering matches error-triage priority (look at events first to diagnose, then decide to retry); `< 40` threshold consistency gives operators and reviewers a single documented narrow-width policy; non-retryable branch is a structural guarantee not a conditional (contentW unused when `retryable=false`); hard switch at threshold is terminal-UI convention with atomic redraw on `WindowSizeMsg`.
...
**Status: PERSONA-EVAL-COMPLETE 2026-04-20** — user-persona returned clean: 0 P1/P2/P3 findings. Positive findings confirm the one-line widen uniformly covers all four zero-activity spinner scenarios FB-103 introduced the thesis for (never-loaded, error-cleared, CRD-absent, successfully-loaded-empty). Switch-case priority proven safe: `activityLoading && len(rows)==0` evaluates before `activityFetchFailed`, so [r] from transient-error correctly shows spinner over error copy for the re-fetch duration. No regressions on any path. **Prior Status: ACCEPTED 2026-04-20** — engineer shipped one-line gate widen at `resourcetable.go:333` from `m.activityRows == nil` to `len(m.activityRows) == 0`; 5 FB-130 tests (AC1 Observable, AC2 component + AC2 model Input-changed pair, AC3/AC4/AC5 Anti-regression for nil/populated/error-state paths) all PASS; revert-and-re-run evidence provided (AC1 + both AC2s FAIL under `== nil` revert, AC3/AC4/AC5 PASS — causally linked to fix); full suite + `go install ./...` clean. **Prior Status: PENDING ENGINEER** — engineer-direct 1-char change at `internal/tui/components/resourcetable.go:333`. Filed 2026-04-20 by product-experience from FB-103 user-persona P3-1.
...
**2026-04-20 update — third data point from FB-134 persona eval:** Persona flagged the same terse/full drift on `[t]` — status bar `[t] table` (chosen Option A, terse convention) vs. help overlay `[t]  quota table` (canonical). Same family as `[3] quota` / `[4] activity` / `[c] ctx`. Persona self-hedged: *"Low priority since `table` is still clearly directional and won't mislead — it just doesn't match the canonical form exactly."* QUOTA-context status bar has room (~72 chars, could fit `[t] quota table` at ~78) but the Option A pick was explicit — status bar terseness is a convention, not a constraint for this specific line. Confirms the pattern is three-key-deep now (`[3]`, `[4]`, `[t]`, `[c]`) and is a design question about status-bar labeling convention, not a bug in any individual key. Still observation-only.

---

### FB-134 — NAV_QUOTA status bar labels `[t]` as `flat` but handler navigates to TablePane

**Status: PERSONA-EVAL-COMPLETE** — filed 2026-04-20 by product-experience from FB-132 user-persona P2 (premise-corrected); routed 2026-04-20 after team-lead queue approval (ahead of FB-071); accepted 2026-04-20 after rework (two submissions — first applied Option B `[t] quota table`, held, corrected to Option A `[t] table` + two additional anti-regression pins + line-11979 stale assertion fix); persona-eval 2026-04-20 returned 1 P3 (terse/canonical drift, folded into FB-133 observation bucket; pattern is now three-key-deep: `[3]`, `[4]`, `[t]`, `[c]`).
