...
// (internal/console/data/resourceclient.go:505-506); here we verify the cmd wire contract.
...
func TestFB005_NoInlineCardRenderers_GrepAssertion(t *testing.T) {
	...
	// Paths relative to internal/console/ (the working directory when this test runs).
	files := []string{
		"model.go",
	...
	for _, f := range files {
		...
		t.Run(f, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("cannot read %s (run tests from repo root with go test ./internal/console/...): %v", f, err)
			}
...
// AC7 | Integration     | go install ./... + go test ./internal/console/...
...
// No third value exists (verified in internal/console/data/severity.go). AC1 covers Error; AC2 covers Warning.
...
// AC10     | [Integration]     | go install ./... + go test ./internal/console/... -count=1
...
// | AC5      | [Integration]     | go install ./... + go test ./internal/console/... -count=1       |
...
// TestViewLineWidths diagnoses the transparent right-side strip by checking
...
func TestViewLineWidths(t *testing.T) {
	...
	borderStyle := lipgloss.NewStyle().
		...
		BorderForeground(lipgloss.Color("#5A6D62")).
		BorderBackground(lipgloss.Color("#0C1D31")).
		Padding(0, 1)

	rendered := borderStyle.Render(inner)
	// Should be: [leftPad(1)][inner(10)][rightPad(1)][border(1)] = 13 chars wide
	w := lipgloss.Width(rendered)
	t.Logf("PaneBorder rendered width: %d (want 13)", w)

	// Check that the last character (│) has an explicit bg in the ANSI output.
	// Match both standalone bg sequences (\x1b[48;...m) and combined fg+bg
	// sequences (\x1b[38;2;R;G;B;48;2;R;G;Bm) that lipgloss may emit.
	bgSet := regexp.MustCompile(`\x1b\[[0-9;]*48;[0-9;]+m`)
	if !bgSet.MatchString(rendered) {
		t.Errorf("no background set in PaneBorder output: %q", rendered)
	}

	// Find the position of │ and the last bg-set sequence before it.
	borderIdx := strings.LastIndex(rendered, "│")
	if borderIdx == -1 {
		t.Fatal("no border char │ in PaneBorder output")
	}
	before := rendered[:borderIdx]
	locs := bgSet.FindAllStringIndex(before, -1)
	if len(locs) == 0 {
		t.Errorf("no bg-set sequence before border │: before=%q", before)
	} else {
		// Check that the last bg before │ is not followed by a reset before │.
		lastBg := locs[len(locs)-1]
		between := rendered[lastBg[1]:borderIdx]
		resetRe := regexp.MustCompile(`\x1b\[(?:0?|;)m`)
		if resetRe.MatchString(between) {
			t.Errorf("bg reset between last bg-set and │: between=%q", between)
		} else {
			t.Logf("OK: bg set before │ with no reset in between")
		}
	}

	// Also check the right padding space before │.
	// Find last space before │.
	rightPadStart := strings.LastIndex(rendered[:borderIdx], " ")
	if rightPadStart == -1 {
		t.Errorf("no space before │")
	} else {
		before2 := rendered[:rightPadStart]
		locs2 := bgSet.FindAllStringIndex(before2, -1)
		if len(locs2) == 0 {
			t.Errorf("no bg-set before right padding space: before=%q", before2)
		} else {
			lastBg2 := locs2[len(locs2)-1]
			between2 := rendered[lastBg2[1]:rightPadStart]
			resetRe := regexp.MustCompile(`\x1b\[(?:0?|;)m`)
			if resetRe.MatchString(between2) {
				t.Errorf("bg reset between last bg-set and right padding space: between=%q", between2)
			} else {
				t.Logf("OK: bg set before right padding space with no reset in between")
			}
		}
	}
}
