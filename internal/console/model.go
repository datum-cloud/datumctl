...
func (m AppModel) recalcLayout() AppModel {
	...
	// Pane-wrapped components receive their inner content width (the area
	// inside PaneBorder's padding + right-border chrome) so their rendering
	...
	m.banner.SetSize(tableInnerW)

	tableH := max(1, tableInnerH-m.banner.Height())
	newTable := components.NewResourceTableModel(tableInnerW, tableH)
	newTable.SetColumns(m.tableColumns, tableInnerW)
	...
	prevDetail := m.detail
	m.detail = components.NewDetailViewModel(tableInnerW, tableInnerH)
	...
	m.quota.SetSize(tableInnerW, tableInnerH)
	m.quota.SetFocused(m.activePane == QuotaDashboardPane)

	m.activity.SetSize(tableInnerW, tableInnerH)
	m.activity.SetFocused(m.activePane == ActivityPane)

	m.activityDashboard.SetSize(tableInnerW, tableInnerH)
	m.activityDashboard.SetFocused(m.activePane == ActivityDashboardPane)

	m.history.SetSize(tableInnerW, tableInnerH)
	m.history.SetFocused(m.activePane == HistoryPane)
...
func (m AppModel) View() tea.View {
	...
	// Paint every line of every row with Surface bg and pad to m.width before
	// joining. JoinVertical pads short lines with plain unstyled spaces that show
	// the terminal background; painting here ensures the full viewport is dark.
	painted := make([]string, len(rows))
	for i, row := range rows {
		h := strings.Count(row, "\n") + 1
		painted[i] = styles.SurfaceFill(row, m.width, h)
	}
