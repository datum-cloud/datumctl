...
func (m QuotaDashboardModel) View() string {
	if m.height < 6 {
		return styles.PaneBorder(m.focused).Render(styles.SurfaceFill(m.buildMainContent(), m.width, m.height))
	}
...
func (m QuotaDashboardModel) renderDashboardTreeRow(
	...
	if selected {
		connector = lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Accent).Render(item.connector)
	}
...
func (m QuotaDashboardModel) titleBar() string {
	...
	warning := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Warning)
