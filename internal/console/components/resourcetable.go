...
func (m ResourceTableModel) View() string {
	...
	case len(m.filteredRows) == 0 && m.filter != "":
		noResults := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Muted).
			Render(fmt.Sprintf("No results for %q", m.filter))
		escHint := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Accent).
	...
	case len(m.filteredRows) == 0:
		content = lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Muted).
			Width(m.tableWidth).Align(lipgloss.Center).
...
func (m ResourceTableModel) renderPlatformHealthSection(contentW int, textOnly, showList bool) string {
	...
	warn := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Warning).Bold(true)
	...
	if m.bucketErr != nil {
		...
		if contentW >= 40 {
			line += "\n" + muted.Render("Press ") + accent.Render("[r]") + muted.Render(" to retry.")
		}
		return leftHeader + "\n\n" + line
	}
...
func (m ResourceTableModel) renderQuickJumpSection(contentW int) string {
	key := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Accent).Bold(true)
	muted := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Muted)
...
func (m ResourceTableModel) renderHeaderBand(contentW int) string {
	...
	if m.forceDashboard && m.typeName != "" {
		accentBold2 := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Accent).Bold(true)
		muted2 := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Muted)
...
func (m ResourceTableModel) renderQuotaRow(r data.ConstrainedRow, width int, textOnly bool) string {
	...
	labelCell := lipgloss.NewStyle().Width(labelW).Render(lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Secondary).Render(label))
...
func (m ResourceTableModel) renderKeybindStrip(contentW int) string {
	key := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Primary).Bold(true)
	muted := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Muted)
...
func (m *ResourceTableModel) SetColumns(cols []string, tableWidth int) {
	...
	m.table.SetStyles(table.Styles{
		Header:   styles.TableStyle.Bold(true).Foreground(styles.Secondary),
		Cell:     lipgloss.NewStyle().Foreground(styles.Primary).Padding(0, 1),
		Selected: styles.SelectedRowStyle.Width(tableWidth),
	})
}
	...
	for _, row := range m.allRows {
		...
		for i, col := range m.columns {
			...
			case "status", "phase", "state", "ready":
				dot := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.StatusColorFor(cells[i])).Render("● ")
				cells[i] = dot + cells[i]
			}
		}
