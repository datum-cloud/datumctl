...
func (m HistoryViewModel) renderRow(
	...
	cursor := "  "
	if selected {
		cursor = lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Accent).Render("▸ ")
	}
...
func (m HistoryViewModel) columnHeader() string {
	revW, timeW, userW, srcW, summaryW := m.columnWidths()
	style := lipgloss.NewStyle().Background(styles.Surface).Foreground(styles.Secondary).Bold(true)
