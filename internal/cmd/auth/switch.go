...
If no email is provided, an interactive picker is shown. If the email
address matches sessions on multiple endpoints, you will be prompted to
choose. Each session remembers the last context you used, so switching
users also restores the context.

The email address must match an account shown by 'datumctl auth list'.
To add a new account, run 'datumctl login' first.`,
	Example: `  # Interactive session picker
