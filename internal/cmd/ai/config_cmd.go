...
func configShowCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			display := cfg
			display.AnthropicAPIKey = redact(cfg.AnthropicAPIKey)
			display.OpenAIAPIKey = redact(cfg.OpenAIAPIKey)
			display.GeminiAPIKey = redact(cfg.GeminiAPIKey)

			out, err := syaml.Marshal(display)
			if err != nil {
				return err
			}
