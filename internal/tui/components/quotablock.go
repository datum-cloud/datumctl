...
func renderBarLine(b data.AllowanceBucket, width int) string {
	...
	if b.Limit == 0 {
		bar := lipgloss.NewStyle().Foreground(styles.Muted).Render(strings.Repeat("░", barWidth))
		counts := lipgloss.NewStyle().Foreground(styles.Muted).
...
func QuotaBarStyling(pctInt int) (barStyle, suffixStyle lipgloss.Style, suffix string) {
	switch {
	case pctInt >= 100:
		s := lipgloss.NewStyle().Foreground(styles.Error).Bold(true)
		return s, s, " ⛔ full"
	case pctInt >= 90:
		s := lipgloss.NewStyle().Foreground(styles.Error)
		return s, s, " ⚠ near"
	case pctInt >= 70:
		s := lipgloss.NewStyle().Foreground(styles.Warning)
		return s, s, ""
	default:
		return lipgloss.NewStyle().Foreground(styles.Success), lipgloss.NewStyle(), ""
	}
}
